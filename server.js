...
// db.js/auth.js가 Supabase 기반 비동기 함수로 바뀌면서, 라우트 핸들러 안에서 발생하는
// rejection을 Express 4가 자동으로 잡아주지 않는다. 각 핸들러를 감싸 next(err)로 넘겨
// 아래의 공통 에러 핸들러(500 응답)가 처리하도록 한다.
...
// 무료 티어 모델(특히 이미지 인식)은 응답이 느릴 수 있어 여유 있게 설정.
// 너무 짧으면 정상적인 응답도 타임아웃으로 실패 처리되어 버린다.
...
async function chatCompletion(model, messages) {
  ...
  try {
    ...
    // AbortSignal.timeout()이 발동하면 "The operation was aborted due to timeout"라는
    // 영어 기술 메시지가 그대로 사용자 화면까지 노출되므로, 원인을 구분해 한국어 안내로 바꾼다.
    if (err.name === "TimeoutError") {
      throw new Error("AI 응답 시간이 초과되었습니다. 잠시 후 다시 시도해주세요.");
    }
    throw new Error("AI 서버와 통신할 수 없습니다. 네트워크 상태를 확인한 뒤 다시 시도해주세요.");
  }
...
app.post("/api/auth/signup", authRateLimit, asyncHandler(async (req, res) => {
  ...
  const user = await db.createUser({ email, passwordHash: hashPassword(password), nickname });
...
app.post("/api/auth/logout", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const header = req.headers.authorization || "";
  await destroySession(header.slice(7));
  return res.json({ success: true });
}));

app.get("/api/profile", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const user = await db.getUserById(req.userId);
  if (!user) return res.status(404).json({ success: false, error: "사용자를 찾을 수 없습니다." });
  return res.json({ success: true, user: publicUser(user) });
}));

app.put("/api/profile", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const nickname = typeof req.body?.nickname === "string" ? req.body.nickname.trim() : "";
  if (!nickname) {
    return res.status(400).json({ success: false, error: "닉네임을 입력해주세요." });
  }
  ...
  const user = await db.updateUserProfile(req.userId, { nickname, dietaryPreferences, allergies });
  if (!user) return res.status(404).json({ success: false, error: "사용자를 찾을 수 없습니다." });
  return res.json({ success: true, user: publicUser(user) });
}));

app.get("/api/folders", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  return res.json({ success: true, folders: await db.listFoldersByUser(req.userId) });
}));

app.post("/api/folders", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  ...
  const folder = await db.createFolder(req.userId, name);
  return res.status(201).json({ success: true, folder });
}));

app.delete("/api/folders/:id", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const result = await db.deleteFolder(req.userId, req.params.id);
  if (!result.success) {
...
app.post("/api/recipes", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  ...
  const saved = await db.addRecipe({ userId: req.userId, recipe, sourceIngredients, folderId });
  return res.status(201).json({ success: true, saved });
}));

app.get("/api/recipes", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const folderId = typeof req.query.folderId === "string" ? req.query.folderId : undefined;
  return res.json({ success: true, recipes: await db.listRecipesByUser(req.userId, folderId) });
}));

app.delete("/api/recipes/:id", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const deleted = await db.deleteRecipe(req.userId, req.params.id);
  if (!deleted) return res.status(404).json({ success: false, error: "레시피를 찾을 수 없습니다." });
  return res.json({ success: true });
}));

app.post("/api/recipes/:id/rate", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const rating = Number(req.body?.rating);
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) {
    return res.status(400).json({ success: false, error: "평점은 1~5 사이의 정수여야 합니다." });
  }
  const updated = await db.rateRecipe(req.userId, req.params.id, rating);
  if (!updated) return res.status(404).json({ success: false, error: "레시피를 찾을 수 없습니다." });
  return res.json({ success: true, recipe: updated });
}));

app.post("/api/recipes/:id/cook", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  const updated = await db.markRecipeCooked(req.userId, req.params.id);
  if (!updated) return res.status(404).json({ success: false, error: "레시피를 찾을 수 없습니다." });
  return res.json({ success: true, recipe: updated });
}));

app.get("/api/dashboard", asyncHandler(requireAuth), asyncHandler(async (req, res) => {
  return res.json({ success: true, stats: await db.getDashboardStats(req.userId) });
}));
...
// Vercel(서버리스)에서는 이 파일을 함수 핸들러로 그대로 import하며 app.listen()을 호출하지
// 않는다. 로컬 개발(`node server.js`)일 때만 포트를 열어 상시 구동 서버로 실행한다.
